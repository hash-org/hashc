...
impl Symbol {
    ...
    pub fn from_name(name: impl Into<Identifier>) -> Self {
        Node::create_at(SymbolData { name: Some(name.into()) }, NodeOrigin::Generated)
    }

    /// Create a new symbol without a name.
    pub fn fresh() -> Self {
        Node::create_at(SymbolData { name: None }, NodeOrigin::Generated)
    }

    /// Create a new symbol with the same name as this one.
    pub fn duplicate(&self) -> Symbol {
        let name = self.borrow().name;
        Node::create_at(SymbolData { name }, NodeOrigin::Generated)
    }

    /// Create a new symbol with an `_` as its name.
    pub fn fresh_underscore() -> Self {
        Node::create_at(SymbolData { name: Some(IDENTS.underscore) }, NodeOrigin::Generated)
    }
