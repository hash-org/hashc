...
//! into some code representation for a backend to process. The current available
//! backends are the following:
//! 
...
//!    The LLVM backend is located in the `hash-codegen-llvm` crate.
//! 
//! 2. The bytecode backend which translates Hash IR into Hash Bytecode so that it
//!   can be processed by the Hash VM. The bytecode backend is located in the
...
//! hand, LLVM backend will emit a runnable executable after the code is generated.
