...
pub(crate) struct TreeDefOpts {
    ...
    /// Used to internally generate the name of the crate in which the
    /// `define_tree` macro is being invoked. This is used as a work around
    /// for autogenerated macros like `ast_visitor_mut_self_default_impl` not
    /// being able to resolve the crate name when the origin crate is the same
    /// name.
    // pub(crate) crate_name: syn::Ident,
