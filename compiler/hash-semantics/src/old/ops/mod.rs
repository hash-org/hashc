...
use hash_tir::old::{builder::PrimitiveBuilder, scope::ScopeId};
...
use crate::old::storage::{AccessToStorage, DiagnosticsStore};

/// Trait to access various structures that can perform typechecking queries,
/// by a reference to a [StorageRef](crate::old::storage::StorageRef).
pub trait AccessToOps: AccessToStorage {
    ...
    /// Create a [DiagnosticsStore]
    fn diagnostics_store(&self) -> &DiagnosticsStore {
        self.diagnostics()
    }
