    ...
    pub(super) fn simplify_match_pair(
        ...
        match *pair.pat.value() {
            ...
            Pat::Range(RangePat { lo, hi, end }) => {
                ...
                // the `lo`
                let range_ty =
                    self.ty_id_from_tir_ty(tir_stores().atom_info().get_inferred_ty(pair.pat));
                ...
                if let Some((min, max, _)) = range {
                    ...
                    // into a u128...
                    let lo_val = self.eval_range_lit(lo, range_ty, false).1 ^ bias;

                    if lo_val <= min {
                        let hi_val = self.eval_range_lit(hi, range_ty, true).1 ^ bias;
