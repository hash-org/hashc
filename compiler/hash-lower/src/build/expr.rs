...
impl<'tcx> Builder<'tcx> {
    ...
    pub(crate) fn term_into_dest(
        ...
        let span = self.get_location(expr).unwrap().span;
        ...
        let block_and = match term {
            ...
            Term::Tuple(TupleTerm { data }) => {
                let ty = self.ty_id_from_tir_term(expr);
            ...
            Term::Prim(prim) => {
                ...
                match prim {
                    PrimTerm::Array(ListCtor { elements }) => {
                        let ty = self.ty_id_from_tir_term(expr);
                        ...
                        let args =
                            self.stores().term_list().map_fast(elements, |elements| {
                                elements
                                .iter()
                                .enumerate()
                                .map(|(index, element)| {
            ...
            Term::Ctor(ref ctor) => {
                ...
                let IrTy::Adt(adt) = self.ty_from_tir_term(expr) else {
            ...
            Term::Decl(decl) => self.lower_declaration(block, &decl, span),
            Term::Assign(_) => {
                ...
                self.control_flow_graph.push_assign(block, destination, const_value.into(),
            span);
    ...
    pub fn fn_call_into_dest(
        ...
        let args = self.stores().args().map_fast(args, |args| {
            args.iter()
                .map(|arg| {
                    unpack!(block = self.as_operand(block, arg.value, Mutability::Immutable))
                })
                .collect::<Vec<_>>()
        });
    ...
    pub fn constructor_into_dest(
        ...
        let CtorTerm { ctor, data_args, ctor_args } = subject;
        ...
        if matches!(aggregate_kind, AggregateKind::Enum(..)) {
            unpack!(block = self.term_into_dest(destination, block, subject));
        }

        let args = self.stores().args().map_fast(*data_args, |args| {
            args.iter()
                .map(|arg| {
