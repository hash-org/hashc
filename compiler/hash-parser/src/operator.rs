...
impl Operator {
    ...
    pub(crate) fn from_token_stream<R: ModuleResolver>(gen: &AstGen<R>) -> (Option<Operator>, u8) {
        ...
        match &(token.unwrap()).kind {
            ...
            TokenKind::Keyword(Keyword::As) => (Some(Operator::As), 1),
            TokenKind::Eq => match gen.peek_second() {
                Some(token) if token.kind == TokenKind::Eq => (Some(Operator::EqEq), 2),
                _ => (None, 0),
            },
            TokenKind::Lt => match gen.peek_second() {
                Some(token) if token.kind == TokenKind::Eq => (Some(Operator::LtEq), 2),
                Some(token) if token.kind == TokenKind::Lt => (Some(Operator::Shl), 2),
                _ => (Some(Operator::Lt), 1),
            },
            TokenKind::Gt => match gen.peek_second() {
                Some(token) if token.kind == TokenKind::Eq => (Some(Operator::GtEq), 2),
                Some(token) if token.kind == TokenKind::Gt => (Some(Operator::Shr), 2),
                _ => (Some(Operator::Gt), 1),
            },
            TokenKind::Plus => (Some(Operator::Add), 1),
            TokenKind::Minus => (Some(Operator::Sub), 1),
            TokenKind::Star => (Some(Operator::Mul), 1),
            TokenKind::Slash => (Some(Operator::Div), 1),
            TokenKind::Percent => (Some(Operator::Mod), 1),
            TokenKind::Caret => match gen.peek_second() {
                Some(token) if token.kind == TokenKind::Caret => (Some(Operator::Exp), 2),
                _ => (Some(Operator::BitXor), 1),
            },
            TokenKind::Amp => match gen.peek_second() {
                Some(token) if token.kind == TokenKind::Amp => (Some(Operator::And), 2),
                _ => (Some(Operator::BitAnd), 1),
            },
            TokenKind::Pipe => match gen.peek_second() {
                Some(token) if token.kind == TokenKind::Pipe => (Some(Operator::Or), 2),
                _ => (Some(Operator::BitOr), 1),
            },
            TokenKind::Exclamation => match gen.peek_second() {
                Some(token) if token.kind == TokenKind::Eq => (Some(Operator::NotEq), 2),
