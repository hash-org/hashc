...
use hash_tir::{
    self,
    ast_info::HasNodeId,
    building::gen::{data_ty, params, params_pos, sym, universe_ty, void_term, void_ty},
...
impl DefinedIntrinsics {
    ...
    pub fn create<T: AccessToEnv + Copy>(env: T) -> Self {
        ...
        let debug_print = {
            let t_sym = sym("T");
            let a_sym = sym("a");
            let params = params([(t_sym, universe_ty(), None), (a_sym, Ty::from(t_sym), None)]);
            let ret = void_ty();
        ...
        let size_of = {
            let t_sym = sym("T");
            let params = params([(t_sym, universe_ty(), None)]);
