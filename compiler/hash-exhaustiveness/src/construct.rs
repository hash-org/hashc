...
use hash_storage::store::{statics::CoreStoreId, SequenceStoreKey, Store};
use hash_tir::{
            ...
            ctor @ (DeconstructedCtor::Single | DeconstructedCtor::Variant(_)) => {
                ...
                match *ctx.ty.value() {
                    Ty::Data(DataTy { data_def, .. }) => {
                        ...
                        CtorDefId(*ctor_id.value(), variant_idx).borrow().params.value().len()
                    }
                    Ty::Tuple(TupleTy { data }) => data.value().len(),
