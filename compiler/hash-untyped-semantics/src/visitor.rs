...
impl<'s> SemanticAnalyser<'s> {
    ...
    // ) { self.append_error( AnalysisErrorKind::InvalidDirectiveArgument { name:
    //   directive.ident, expected, received: subject.body().into(), notes: vec![],
    //   }, subject, )
    // }
    ...
    //     directive: AstNodeRef<Name>,
    //     subject: AstNodeRef<Expr>,
    // ) { // Check that the supplied argument to `#entry_point` is a // declaration, the entry
    //   point must be a function definition // which will be checked at a later stage.
    ...
    //     directive: AstNodeRef<Name>,
    //     subject: AstNodeRef<Expr>,
    // ) { if let Expr::Declaration(decl) = subject.body() { if decl.value.is_none() {
    //   self.invalid_argument(DirectiveArgument::Declaration, directive, subject); return; }
    ...
    // ) -> Result<(), Infallible> { let name = directive.ident;
    ...
    //                         ) { notes.push( "currently, the `#layout_of` directive does not
    //                           handle function definitions. This is subject to change in the
    //                           future.".to_string(), );
    //                         }
    //                         self.append_error(
    //                             AnalysisErrorKind::InvalidDirectiveArgument {
    ...
    //                     notes.push(
    //                         "`#layout_of` requires that the provided declaration must have a
    // value"                             .to_string(),
    //                     )
    //                 };
    //                 self.append_error(
